...
if (!$isAlreadyLoggedIn && isset($_SESSION['logged_in']) && $_SESSION['logged_in'] === true) {
    ...
    // 檢查角色是否允許進入後台（管理員、行政人員、主任、老師、IM）
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'login') {
    ...
    try {
        ...
        // 2. 驗證密碼
            ...
            if ($login_successful) {
                // 優先檢查角色是否不允許登入後台（允許管理員、行政人員、主任、科助、招生中心組員、資管科主任、科助）
                // 使用角色代碼：'ADM'=管理員, 'STA'=行政人員, 'DI'=主任, 'TEA'=科助, 'STAM'=招生中心組員, 'IM'=資管科主任, 'AS'=科助
                $allowed_roles = ['ADM', 'STA', 'DI', 'TEA', 'STAM', 'IM', 'AS'];
                // 向後兼容：也檢查舊的中文角色名稱
                $allowed_role_names = ['管理員', '行政人員', '主任', '老師', '招生中心組員', '資管科主任', '科助'];
